...
it("adds noindex robots meta to the not-found page and omits it for normal routes", async () => {
  ...
  expect(homeHtml).not.toContain('<meta name="robots" content="noindex">');
  expect(homeHtml).not.toContain("<!--page-robots-->");
});
...
it("writes the home page with page-specific metadata", async () => {
  await prerender({ base: "/brand/", outDir, template, render });
  const home = await readFile(join(outDir, "index.html"), "utf8");
