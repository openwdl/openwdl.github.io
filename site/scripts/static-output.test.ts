 ...
 *
 *   OPENWDL_BASE=/brand/ npm run build -w @openwdl/site
 *     OR
...
// The wizard was removed outright and the community page is kept as source but
// left unrouted, so the build must emit neither.
it("emits no get-started document", async () => {
  await expect(readBuiltRoute(dist, "/get-started/")).rejects.toThrow();
});
...
// ── Home and About redirect ───────────────────────────────────────────────────

it("prerenders the home page with metadata and canonical URL", async () => {
  const html = await readBuiltRoute(dist, "/");
  const basePath = base.slice(0, -1);

  expect(html).not.toContain("<!--app-html-->");
  expect(html).toContain('data-page-id="home"');
  expect(html).toContain('window.__OPENWDL_PAGE_ID__="home"');
  expect(html).toContain("<title>OpenWDL | Workflow Description Language</title>");
  expect(html).toContain(`href="https://openwdl.org${basePath}/"`);
...
// The page's source is kept in the repository but deliberately unrouted, so the
// build must not emit it at all.
it("emits no community document", async () => {
  await expect(readBuiltRoute(dist, "/community/")).rejects.toThrow();
});
