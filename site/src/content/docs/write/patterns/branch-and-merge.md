...
description: "Fan out task outputs into parallel paths then recombine the results."
slug: /docs/start/patterns/branch-and-merge/
section: learn
...
chaining](/docs/start/patterns/linear-chaining/) and [Multiple
I/O](/docs/start/patterns/multiple-io/) chapters can be further extended to direct a
...
computations before finally executing (the merge).](/docs/patterns/branch-and-merge/header.png)
...
workflow run {
  ...
  # Run `stepC`, connecting the `value` input to `stepA`'s `out` output.
  call stepC { value = stepA.out }

  # Run `stepD`, connecting the `in1` input to `stepB`'s `out` output
  # and the `in2` input to `stepC`'s `out` output.
  call stepD { in1 = stepB.out, in2 = stepC.out }
}
```

## Continuing our example

To build on the example started in [Multiple I/O](/docs/start/patterns/multiple-io/#an-example),
...
workflow run {
  ...
  call filter_snps { vcf = split_vcf.snp_vcf }
  call filter_indels { vcf = split_vcf.indel_vcf }
