...
| Path                     | Contents                                                      |
| ------------------------ | ------------------------------------------------------------- |
| `site/`                  | The website application (npm workspace `@openwdl/brand`).      |
...
```sh
npm run build -w @openwdl/brand        # full static build into site/dist
npm test -w @openwdl/brand             # unit and component tests
npm run test:static -w @openwdl/brand  # assertions against the built output
npm run lint -w @openwdl/brand         # eslint
```
...
© 2019-Present The OpenWDL Developers. See [LICENSE](LICENSE).
